...
class RepositorySearchBloc extends Bloc<RepositorySearchEvent, RepositorySearchState> {
  RepositorySearchBloc({required this.repositoryOfRepository})
      : super(const RepositorySearchState(searchTerm: "", page: 1,));

  final RepositoryOfRepository repositoryOfRepository;

  @override
  Stream<Transition<RepositorySearchEvent, RepositorySearchState>> transformEvents(
  ...
  @override
  Stream<RepositorySearchState> mapEventToState(RepositorySearchEvent event) async* {
  ...
  Future<RepositorySearchState> _mapLoadMoreRepositoryToState(RepositorySearchState state) async {
    try {
      var results;
      var page = state.page;
      page++;
      if (state.status == LoadStateStatus.success) {
        results = await repositoryOfRepository.repositorySearch(state.searchTerm!, page);
      }
  ...
  Future<RepositorySearchState> _mapClearRepositoryToState(RepositorySearchState state) async {
    return state.copyWith(
      hasReachedMax: false,
      searchTerm: '',
  ...
  Future<RepositorySearchState> _mapNextPageToState(RepositorySearchState state) async {
    try {
      var results;
      var page = state.page;
      page++;
  ...
  Future<RepositorySearchState> _mapPreviousPageToState(RepositorySearchState state) async {
    try {
      var results;
      var page = state.page;
      if (page <= 1) {
